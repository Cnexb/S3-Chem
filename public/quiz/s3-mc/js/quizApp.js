...
const UI = {
 en: {
 appSubtitle: "S3 Chemistry MCQ · Earth & Microscopic World · English / 繁體中文",
 hSettings: "Worksheet settings",
 lblTypes: "Quiz sets",
 bankNone: "Select at least one quiz set.",
 ...
 revBandFair: "Mixed performance: re-read S3 Chemistry MCQ notes for weaker topics, then regenerate.",
 revBandLow:
 ...
 lblCount: "整组（固定次序）",
 btnGenerate: "显示所选题组",
 txtGenHint: "可多选代号（如 Earth1Aa）。所选题组的全部题目会按题库固定次序显示。",
 hExport: "导出",
 ...
 lblCount: "整組（固定次序）",
 btnGenerate: "顯示所選題組",
 txtGenHint: "可多選代號（如 Earth1Aa）。所選題組的全部題目會按題庫固定次序顯示。",
 hExport: "匯出",
...
export function initQuiz() {
 ...
 if (!sections.length) {
 els.bankSummary.innerHTML = ` ${escHtml(t("bankSummaryTitle"))} ${escHtml(t("bankNone"))} `;
 return;
 }
 ...
 return ` ${escHtml(label)} ${n} `;
 })
 .join("");

 els.bankSummary.innerHTML = `
 ${escHtml(t("bankSummaryTitle"))} 
 ${escHtml(t("bankAvailable"))} 
 ${stats.total} 
 
 ${escHtml(t("bankByTopic"))} 
 ${topicRows} 
 `;
 }
 ...
 updateBankSummary();
 // Selecting sets loads all questions from those sets in fixed bank order.
 if (selectedSections().length) generate();
 else {
 lastQuestions = [];
 attemptMap.clear();
 updateProgress();
 ...
 function initMeta() {
 const prev = new Set(
 Array.from(els.typeChecks?.querySelectorAll("input[type='checkbox']:checked") || []).map((x) => x.value)
 );
 ...
 return Array.from(els.typeChecks?.querySelectorAll("input[type='checkbox']:checked") || []).map((x) => x.value);
 }
 ...
 // Fixed bank order: every question in the chosen set(s), no shuffle / no count cap.
 const sectionOrder = QUIZ_SECTIONS.map((s) => s.id).filter((id) => sections.includes(id));
 const pool = [];
 ...
 bank.innerHTML = ` Word bank: ${escHtml(q.wordBank.join(" · "))}`;
 ...
 fb.innerHTML = ` ${escHtml(t("modelPrefix"))} ${escHtml(modelAnswerForLang(q, lang))}`;
 };
