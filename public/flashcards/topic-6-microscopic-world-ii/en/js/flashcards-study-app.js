...
const CHAPTER_TITLE = "Topic 6: Microscopic World II";
const CHAPTER_SUBTITLE = "HKDSE Chemistry · Shapes, electronegativity, polarity, intermolecular forces";
...
function renderFilters() {
  ...
  const chips = [
    `<button type="button" class="filter-chip active px-3 py-1.5 rounded-full border border-outline-variant/60 text-on-surface-variant font-label-bold" data-filter="">All</button>`,
    ...subtopics.map(
      (st) =>
...
function renderCard() {
  ...
  if (!card) {
    els.cardFront.textContent = session.sessionComplete
      ? "All cards mastered in this round!"
      : "No cards to display";
  ...
  if (card.backCompact) {
    els.cardBack.classList.add("card-text-long");
  ...
  els.ratingHint.textContent = session.flipped
    ? "← Again · Got it → or press 1 / 2"
    : "Click card, Space, or FLIP to show answer";
...
function renderSummary() {
  ...
  if (session.sessionComplete) {
    ...
    els.summaryStats.textContent =
      "All cards have reached mastery weight — restart or switch filters.";
  } else {
